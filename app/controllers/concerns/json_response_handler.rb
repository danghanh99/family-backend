module JsonResponseHandler
  ...
  def render_resource(obj, status_code_symbol = :ok, serializer_class = nil)
    if serializer_class.nil?
      ...
      (render json: { "#{obj.class.to_s.underscore}": serializer_class.new(obj) },
              status: status_code_symbol)
    end
  end

  def render_resource_event(event, child_events, status_code_symbol = :ok, serializer_class = nil)
    if serializer_class.nil?
      (render json: event,
              status: status_code_symbol)
    else
      (render json: { "#{event.class.to_s.underscore}": serializer_class.new(event), 'child_events': serializer_class.new(child_events) },
              status: status_code_symbol)
    end
  end
  ...
  def render_resources(objs, status_code_symbol, serializer_class = nil)
    if serializer_class.nil?
      ...
      (render json: objs,
              each_serializer: serializer_class, status: status_code_symbol)
    end
  end
end
